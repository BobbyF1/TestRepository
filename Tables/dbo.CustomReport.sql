CREATE TABLE [dbo].[CustomReport]
(
[Id] [int] NOT NULL IDENTITY(1, 1),
[Name] [varchar] (256) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL,
[Enabled] [char] (1) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL,
[Description] [varchar] (512) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
[creation_date] [datetime] NOT NULL CONSTRAINT [DF_CustomReport_creation_date] DEFAULT (getdate()),
[last_updated_date] [datetime] NOT NULL CONSTRAINT [DF_CustomReport_last_updated_date] DEFAULT (getdate())
) ON [PRIMARY]
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO

CREATE TRIGGER [dbo].[trg_CustomReport_updated]
ON [dbo].[CustomReport]
FOR UPDATE
AS
--start_autogenerated_trigger_text  - do not edit comments or code!
IF (suser_name() = 'svc_sync_user') OR (suser_name() = 'svc_agent_user')  RETURN
--end_autogenerated_trigger_text

BEGIN

 SET NOCOUNT ON 

 UPDATE dbo.CustomReport SET dbo.CustomReport.last_updated_date = GETDATE()
 FROM inserted
 WHERE inserted.Id = dbo.CustomReport.Id

END
GO
ALTER TABLE [dbo].[CustomReport] ADD CONSTRAINT [PK_CustomReport] PRIMARY KEY CLUSTERED  ([Id]) ON [PRIMARY]
GO
